...
class BlockUserFragment : Fragment() {
    ...
    private fun getChip(blockUserEntity: BlockUserEntity): Chip {
        val chip = Chip(requireContext())
        val paddingDp = TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_DIP, 5f,
            resources.displayMetrics
        ).toInt()
        chip.text = "${blockUserEntity.name} ${blockUserEntity.account}"

        chip.setOnLongClickListener {
            runBlocking {
                viewModel.deleteSingleUser(blockUserEntity)
                getUserList()
            }
